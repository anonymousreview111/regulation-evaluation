...
weakly served or unserved by technical evaluation methods, that is, where is the
...
dates are released for reproducibility in the companion artifact. As a transparency check, a
June~2026 re-run of the governance$\times$evaluation query on arXiv returned 168 records; the
original per-database candidate yields were not separately retained, and Scopus and Web of
Science require institutional access (the released query strings permit re-running).
...
chiefly asks for, for example an adversarial-robustness report for the EU AI Act Article~15
robustness facet. \emph{Supporting} (\partialev): the artifact feeds a required outcome or
document but is insufficient alone, as when a benchmark result is an input to the Article~9
risk-management system but does not constitute it. \emph{Contextual} (\indirect): the
artifact informs the requirement without serving as its evidence, as when interpretability
...
\emph{External validation.} As an external check, a validator who is not an author of this
study (acknowledged below), blind to the first-pass codes,
...
primary$\leftrightarrow$contextual conflicts. The external recoding
